...
class EmpresaDAO extends Conexao{
    ...
    public function ConsultarEmpresa(){
        ...
        $conexao = parent::retornaConexao();
        $comando_sql = 'Select id_empresa, nome_empresa, endereco_empresa, telefone_empresa, email_empresa from tb_empresa where id_usuario = ?';
        $sql = $conexao->prepare($comando_sql);
       ...
       public function DetalharEmpresa($id_Emp){
        ...
        $conexao = parent::retornaConexao();
        $comando_sql = 'Select id_empresa, nome_empresa, endereco_empresa, telefone_empresa, email_empresa from tb_empresa where id_empresa = ? and id_usuario = ?';
        $sql = $conexao->prepare($comando_sql);
    ...
    public function AlterarEmpresa($nome,$endereco,$telefone,$email, $cod){
        ...
        $conexao = parent::retornaConexao();
        $comando_sql = 'update tb_empresa set nome_empresa = ?, endereco_empresa = ?, telefone_empresa = ?, email_empresa = ? where id_empresa = ? and id_usuario = ?';
        $sql = $conexao->prepare($comando_sql);
        $sql->bindValue(1, $nome);
